---
title: 路径分析与结构方程模型
author:
 - 授课教师：吴翔 \newline
 - 邮箱：wuhsiang@hust.edu.cn
date: "Sept 23, 2021"
linestretch: 1.25
fontsize: 18
header-includes:
  - \usepackage{ctex}
output:
  beamer_presentation:
    theme: "CambridgeUS"
    colortheme: "beaver"
    latex_engine: xelatex
    toc: true
---

```{r setup, include = FALSE}

knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, fig.height = 3, fig.width = 5)
options(digits = 2)

suppressMessages(library(ggplot2))
suppressMessages(library(magrittr))
suppressMessages(library(stargazer))

```


# 因果推断概述


## 案例讨论：谁能影响战争结局？

- **官渡之战**：“昔许子远舍袁就曹，规画计较，应见纳受，遂破袁军，以定曹业。向使曹氏不信子远，怀疑犹豫，不决於心，则今天下袁氏有也。” --《三国志·是仪胡综传》
- **赤壁之战**：后太祖征荆州还，於巴丘遇疾疫，烧船，叹曰：“郭奉孝在，不使孤至此。” --《三国志·郭嘉传》
- **夷陵之战**：先主既即尊号，将东征孙权以复关羽之耻，群臣多谏，一不从。章武二年，大军败绩，还住白帝。亮叹曰：“法孝直若在，则能制主上，令不东行；就复东行，必不倾危矣。” --《三国志·法正传》

请思考并讨论，（1）按照《三国志》中观点，东汉末年三次大战，谁扭转了战局？其**逻辑**是什么？（2）赤壁之战前夕，谁游说了孙权联刘抗曹？

## 案例讨论：赤壁

\begin{quotation}
  折戟沉沙铁未销，自将磨洗认前朝。\\
  东风\textcolor{red}{不}与周郎便，铜雀春深锁二乔。\\
  --杜牧《赤壁》
\end{quotation}

## 案例讨论：大运河

\begin{quotation}
万艘龙舸绿丝间，载到扬州尽不还。\\
应是天教开汴水，一千余里地无山。\\

尽道隋亡为此河，至今千里赖通波。\\
\textcolor{red}{若无}水殿龙舟事，共禹论功不较多。\\
  --皮日休《汴河怀古二首》
\end{quotation}


## 案例讨论：辨真伪

\begin{quotation}
赠君一法决狐疑，不用钻龟与祝蓍。\\
试玉要烧三日满，辨材须待七年期。\\
周公恐惧流言日，王莽谦恭未篡时。\\
\textcolor{red}{向使}当初身便死，一生真伪复谁知？\\
  --白居易《放言五首·其三》
\end{quotation}

## 如何归因？


## 归因模型



## 如何评估因果效应？



# 因果推断的反事实框架

## 反事实与潜在结果





## Rubin因果模型

Rubin因果模型（Rubin Causal Model, RCM）逻辑如下。设$X_{i}$表示个体$i$是否接受处理或干预，$X_{i}=1$代表处理，$X_{i}=0$代表对照，而$Y_{i}$代表观测到的结果变量。

另外，记$\{Y_{i}(1), Y_{i}(0)\}$代表个体$i$接受处理或者对照的潜在结果，那么个体$i$接受处理的因果效应即为：$Y_{i}(1)-Y_{i}(0)$。

总体的平均因果效应（average causal effect, ACE）如下：
$$
ACE(X \rightarrow Y) = E[Y_{i}(1)-Y_{i}(0)].
$$

## Rubin因果模型（续）

不幸的是，个体$i$要么接受处理，要么接受对照，因此两个潜在结果$\{Y_{i}(1), Y_{i}(0)\}$必然缺失一个，而只能观测到其中之一。因此，观测结果
$$
Y_{i}=
\begin{cases}
Y_{i}(1),\text{ if } X_{i} = 1;\\
Y_{i}(0), \text{ if } X_{i} = 0.
\end{cases}
$$
或者表示为，
$$
Y_{i} = X_{i} \cdot Y_{i}(1) + (1 - X_{i}) \cdot Y_{i}(0).
$$

## Rubin因果模型（续）

问题在于，真实的平均因果效应$E[Y_{i}(1)-Y_{i}(0)]$与观测到的组间差异$E(Y_{i}|X_{i}=1) - E(Y_{i}|X_{i}=0)$是什么关系？

## Rubin因果模型（续）

个体$i$接受处理的因果效应可以分解为两部分，
$$
Y_{i}(1) - Y_{i}(0) = [Y_{i}(1) - Y_{i}|X_{i}=1] + [Y_{i}|X_{i}=1 - Y_{i}(0)]
$$



# 路径模型概述


## 蒙提霍尔悖论

美国电视娱乐节目Let’s Make a Deal的主持人Monty Hall:

- 你面前有三扇关闭着的门，其中一扇门后是汽车，其余两扇门后是山羊，选中后面有车的那扇门就可以赢得该汽车。假设你选定了A门，主持人开启了C门，露出一只山羊。那么，你是否要更换选择，选B门？

在流行病领域，类似的问题称为博克森悖论：两种疾病即使在一般人群中彼此不存在实际联系，在医院的病人中也会形成某种似是而非的关联。

## 讨论：如何抉择？


![Monty Hall Problem](figures/montyhall.png){width=80%}


## 三种基本结构

![基本结构](figures/dagstructure.png){width=60%}


## 链结构案例

考虑$X \rightarrow Y \rightarrow Z$这一链结构，

```{r, echo = TRUE}

# clean the work directory
rm(list = ls())
# number of observations
n <- 300
# generate variables
x <- rnorm(n = n, mean = 110, sd = 10)
y <- -0.5 + 0.2 * x + rnorm(n = n, mean = 0, sd = 2)
z <- 2 + 5 * y + rnorm(n = n, mean = 0, sd = 5)
# combine three variables
dat <- data.frame(x = x, y = y, z = z)

```

## 链结构案例（续）


```{r}

# scatter plot
ggplot(dat, aes(x = x, y = z)) + geom_point() + geom_smooth(method=lm, color="darkred", fill="blue") + theme_bw()

```

## 链结构案例（续）


```{r, results='asis'}

fit1 <- lm(z ~ x, data = dat)
fit2 <- lm(z ~ x + y, data = dat)
# output as a table
stargazer(fit1, fit2, type = "latex", title = "链结构回归分析案例", header = FALSE, single.row = TRUE, digits = 2, keep.stat = c("rsq", "f"))

```


## 分叉结构

考虑$Z \leftarrow X \rightarrow Y$这一分叉结构，

```{r, echo = TRUE}

# clean the work directory
rm(list = ls())
# number of observations
n <- 300
# generate variables
x <- rnorm(n = n, mean = 110, sd = 10)
y <- -0.5 + 0.2 * x + rnorm(n = n, mean = 0, sd = 2)
z <- 2 + 5 * x + rnorm(n = n, mean = 0, sd = 5)
# combine three variables
dat <- data.frame(x = x, y = y, z = z)

```


## 分叉结构案例（续）


```{r}

# scatter plot
ggplot(dat, aes(x = y, y = z)) + geom_point() + geom_smooth(method=lm, color="darkred", fill="blue") + theme_bw()

```

## 分叉结构案例（续）


```{r, results='asis'}

fit1 <- lm(z ~ y, data = dat)
fit2 <- lm(z ~ y + x, data = dat)
# output as a table
stargazer(fit1, fit2, type = "latex", title = "分叉结构回归分析案例", header = FALSE, single.row = TRUE, digits = 2, keep.stat = c("rsq", "f"))

```

## 对撞结构

考虑$X \rightarrow Z \leftarrow Y$这一对撞结构，

```{r, echo = TRUE}

# clean the work directory
rm(list = ls())
# number of observations
n <- 300
# generate variables
x <- rnorm(n = n, mean = 110, sd = 10)
y <- rnorm(n = n, mean = 60, sd = 12)
z <- 2 + 5 * x - 5 * y + rnorm(n = n, mean = 0, sd = 5)
# combine three variables
dat <- data.frame(x = x, y = y, z = z)

```


## 对撞结构案例（续）


```{r}

# scatter plot
ggplot(dat, aes(x = x, y = y)) + geom_point() + geom_smooth(method=lm, color="darkred", fill="blue") + theme_bw()

```

## 分叉结构案例（续）


```{r, results='asis'}

fit1 <- lm(y ~ x, data = dat)
fit2 <- lm(y ~ x + z, data = dat)
# output as a table
stargazer(fit1, fit2, type = "latex", title = "对撞结构回归分析案例", header = FALSE, single.row = TRUE, digits = 2, keep.stat = c("rsq", "f"))

```
